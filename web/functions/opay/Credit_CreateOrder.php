<?php
 ...
 $total_amount = $customer_order['tatal'] + $customer_order['shipping'];
 ...
 $order_details = $query2->fetchAll(PDO::FETCH_ASSOC);
    ...
    try {
        ...
        //基本參數(請依系統規劃自行調整)
        $obj->Send['ReturnURL']         = 'http://localhost/cake_house/web/order_success.php'; //付款完成通知回傳的網址
        $obj->Send['MerchantTradeNo']   = $customer_order['order_no'] ;                                         //訂單編號
        $obj->Send['MerchantTradeDate'] = date('Y/m/d H:i:s');                                    //交易時間
        $obj->Send['TotalAmount']       = $total_amount;                                                   //交易金額
        $obj->Send['TradeDesc']         = "Cake House訂單".$customer_order['order_no'] ;                                       //交易描述
